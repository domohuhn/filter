    ...
    GIVEN( "An options structure filled with values for IIR exponential lowpass filters" ) {
        ...
        WHEN( "the create function is called" ) {
            ...
            THEN( "the buffers are initialized" ) {
                ...
                REQUIRE((void*)iir_exp.inputs == (iir_exp.buffer + 1 * sizeof(DH_FILTER_VALUE_TYPE)));
                REQUIRE((void*)iir_exp.coefficients_out == (iir_exp.buffer + 2 * sizeof(DH_FILTER_VALUE_TYPE)));
                REQUIRE((void*)iir_exp.outputs == (iir_exp.buffer + 4 * sizeof(DH_FILTER_VALUE_TYPE)));
                REQUIRE(iir_exp.current_input_index == 0);
                REQUIRE(iir_exp.current_output_index == 0);
                REQUIRE(iir_exp.buffer_length == 6*sizeof(DH_FILTER_VALUE_TYPE));
